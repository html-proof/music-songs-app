...
class _PlayerAudioHandler extends BaseAudioHandler
  ...
  Future<SeekResponse> customPlayerSeek(SeekRequest request) async {
    await _performSeek(request.position ?? Duration.zero, index: request.index);
    return SeekResponse();
  }
  ...
  Future<void> skipToQueueItem(int index) async {
    await _performSeek(Duration.zero, index: index);
  }
  ...
  @override
  Future<void> seek(Duration position) async => await _performSeek(position);
  ...
  Future<void> _seekRelative(Duration offset) async {
    await _performSeek(currentPosition + offset);
  }
  ...
  void _seekContinuously(bool begin, int direction) {
    _seeker?.stop();
    final duration = _seekDurationForIndex();
  ...
  Future<void> _performSeek(Duration position, {int? index}) async {
    final clampedPosition = _clampPosition(position, index: index);
    await _seekLock.synchronized(() async {
      final oldIndex = _justAudioEvent.currentIndex;
      // Optimistically update state so notification/app UI reflects the seek
      // immediately, while the platform seek is processed.
      _justAudioEvent = _justAudioEvent.copyWith(
        updatePosition: clampedPosition,
        updateTime: DateTime.now(),
        currentIndex: index ?? _justAudioEvent.currentIndex,
      );
      // When the track index changes (e.g. skip from notification), update
      // the notification media item to show the correct song info.
      if (index != null && index != oldIndex) {
        final targetIndex = index;
        if (targetIndex >= 0 && targetIndex < currentQueue.length) {
          mediaItem.add(currentQueue[targetIndex]);
        }
      }
      _broadcastStateIfActive();
      await (await _player)
          .seek(SeekRequest(position: clampedPosition, index: index));
      // Post-seek broadcast ensures the progress bar settles at the exact
      // position after any platform-side buffering or adjustment.
      _broadcastStateIfActive();
    });
  }
  ...
  Duration _clampPosition(Duration position, {int? index}) {
    ...
    final duration = _seekDurationForIndex(index: index);
    if (duration != null && clampedPosition > duration) {
      clampedPosition = duration;
    }
    return clampedPosition;
  }

  Duration? _seekDurationForIndex({int? index}) {
    final queueIndex = index ?? _justAudioEvent.currentIndex;
    if (queueIndex != null &&
        queueIndex >= 0 &&
        queueIndex < currentQueue.length) {
      return currentQueue[queueIndex].duration;
    }
    return currentMediaItem?.duration ?? _justAudioEvent.duration;
  }
  ...
  void _broadcastState() {
    ...
    // Android compact notification supports at most 3 action buttons.
    final compactIndices = List.generate(
      min(controls.length, 3),
      (i) => i,
    );
    playbackState.add(playbackState.nvalue!.copyWith(
      controls: controls,
      // Enable seek & skip actions so the notification progress bar,
      // lock screen, Bluetooth headsets, and car displays all work.
      systemActions: {
        MediaAction.seek,
...
class _Seeker {
  ...
  Future<void> start() async {
    ...
    while (_running) {
      ...
      await handler.seek(newPosition);
